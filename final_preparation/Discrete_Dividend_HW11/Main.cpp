...
void BTTest() {
	...
	tol = 10.1;
	N = 6;
	OLD = SOpt.European_FixDiv("PUT", N, 2./12, v_div);
	while (tol >= pow(10, -4)) {
		N = 2 * N;
	...
	tol = 10.1;
	N = 7;
	OLD = LOpt.European_FixDiv_Mult("PUT", N, times, v_div);
	while (tol >= pow(10, -4)) {
		N = 2 * N;
		NEW = LOpt.European_FixDiv_Mult("PUT", N, times, v_div);
		tol = abs(NEW[0] - OLD[0]);
		OLD = NEW;
		cout << tol << endl;
	}
	...
	tol = 10.1;
	N = 7;
	OLD = LOpt.American_FixDiv_Mult("PUT", N, times, v_div);
	while (tol >= pow(10, -4)) {
		N = 2 * N;
		NEW = LOpt.American_FixDiv_Mult("PUT", N, times, v_div);
		tol = abs(NEW[0] - OLD[0]);
		OLD = NEW;
		cout << tol << endl;
	}
...
void MCTest() {
	...
	vector<double> div_values = { 0.95, 0.02, 0.45 };
